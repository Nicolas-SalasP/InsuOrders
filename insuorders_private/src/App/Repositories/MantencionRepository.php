...
class MantencionRepository
{
    ...
    {
        $this->db->prepare("INSERT INTO activos_docs (activo_id, nombre_archivo, url_archivo) 
                VALUES (:id, :nom, :url)")->execute([':id' => $activoId, ':nom' => $nombre, ':url' => $url]);
    }
    ...
    public function getKitActivo($activoId)
    {
        $sql = "SELECT ai.insumo_id as id, i.nombre, i.codigo_sku, ai.cantidad_default as cantidad, 
                ...
                FROM activos_insumos ai JOIN insumos i ON ai.insumo_id = i.id 
    ...
    public function addInsumoToKit($activoId, $insumoId, $cant)
    {
        ...
        $sql = $check->fetch() ?
            "UPDATE activos_insumos SET cantidad_default = :c WHERE activo_id=:a AND insumo_id=:i" :
    ...
    {
        $this->db->prepare("DELETE FROM activos_insumos WHERE activo_id=:a AND insumo_id=:i")
            ->execute([':a' => $activoId, ':i' => $insumoId]);
        $this->recalcularMinimoInsumo($insumoId);
    }

    public function updateKitQuantity($activoId, $insumoId, $cantidad)
    {
        $this->db->prepare("UPDATE activos_insumos SET cantidad_default = :c WHERE activo_id = :a AND insumo_id = :i")
            ->execute([':c' => $cantidad, ':a' => $activoId, ':i' => $insumoId]);
        $this->recalcularMinimoInsumo($insumoId);
    }
    ...
    private function recalcularMinimoInsumo($insumoId)
    {
        ...
        $this->db->prepare("UPDATE insumos SET stock_minimo = :min WHERE id = :id")
            ->execute([':min' => $total, ':id' => $insumoId]);
    }
    ...
    public function getDetallesOT($id)
    {
        $sql = "SELECT d.id as detalle_id, d.insumo_id as id, d.cantidad, d.cantidad_entregada, d.estado_linea, 
                ...
                (SELECT GROUP_CONCAT(DISTINCT emp.nombre_completo SEPARATOR ', ') 
                FROM movimientos_inventario mi 
                JOIN empleados emp ON mi.empleado_id = emp.id 
                WHERE mi.referencia_id = d.id AND mi.tipo_movimiento_id = 2) as retirado_por
    ...
    public function finalizar($id)
    {
        ...
        try {
            ...
            $this->db->prepare("UPDATE insumos i JOIN detalle_solicitud ds ON i.id = ds.insumo_id 
                                SET i.stock_actual = i.stock_actual + (ds.cantidad - ds.cantidad_entregada) 
                                WHERE ds.solicitud_id = :id AND ds.estado_linea = 'RESERVADO'")->execute([':id' => $id]);
            $this->db->prepare("UPDATE detalle_solicitud SET estado_linea = 'CANCELADO' 
                                WHERE solicitud_id = :id AND estado_linea NOT IN ('ENTREGADO', 'FINALIZADO')")
                ->execute([':id' => $id]);
    ...
    public function delete($id)
    {
        ...
        try {
            ...
            $this->db->prepare("UPDATE detalle_solicitud SET estado_linea = 'ANULADO' WHERE solicitud_id = :id")
                ->execute([':id' => $id]);
            if (!$inTransaction)
                $this->db->commit();
        } catch (Exception $e) {
        ...
        try {
            ...
            foreach ($ubicaciones as $ubi) {
                ...
                $this->db->prepare("INSERT INTO movimientos_inventario (insumo_id, tipo_movimiento_id, cantidad, usuario_id, observacion, referencia_id, empleado_id, ubicacion_id, fecha) 
                    ...
                    ->execute([':iid' => $linea['insumo_id'], ':cant' => $descuento, ':uid' => $usuarioId, ':ref' => $detalleId, ':emp' => $receptorId, ':ubi' => $ubi['ubicacion_id']]);
    ...
    public function getEntregasOT($otId)
    {
        $sql = "SELECT m.fecha, m.cantidad, i.nombre, i.codigo_sku, e.nombre_completo as receptor 
                FROM movimientos_inventario m JOIN insumos i ON m.insumo_id = i.id 
                LEFT JOIN empleados e ON m.empleado_id = e.id 
                WHERE m.referencia_id IN (SELECT id FROM detalle_solicitud WHERE solicitud_id = :id) 
    ...
    public function devolverMaterial($detalleId, $cantidadDevolver, $bodegueroId, $ubicacionDestinoId = 1)
    {
        ...
        $nuevoEstado = ($nuevaEntregada < floatval($linea['cantidad'])) ? 'PENDIENTE' : 'ENTREGADO';
        if ($nuevaEntregada <= 0)
        ...
        $this->db->prepare("UPDATE solicitudes_ot SET estado_id = 1 WHERE id = :id AND estado_id = 2")
            ->execute([':id' => $linea['solicitud_id']]);
        ...
        $this->db->prepare($sqlStock)->execute([
            ':iid' => $linea['insumo_id'],
            ':uid' => $ubicacionDestinoId,
            ':cant' => $cantidadDevolver,
            ':cant_upd' => $cantidadDevolver
        ]);

        $this->db->prepare("INSERT INTO movimientos_inventario (insumo_id, tipo_movimiento_id, cantidad, usuario_id, observacion, referencia_id, ubicacion_id, fecha) 
                        VALUES (:iid, 1, :cant, :uid, 'Devolución de OT', :ref, :ubi, NOW())")
            ->execute([
                ':iid' => $linea['insumo_id'],
                ':cant' => $cantidadDevolver,
                ':uid' => $bodegueroId,
                ':ref' => $detalleId,
                ':ubi' => $ubicacionDestinoId
            ]);
