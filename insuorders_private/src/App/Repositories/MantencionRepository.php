...
class MantencionRepository
{
    ...
    {
        $sql = "SELECT ai.insumo_id as id, i.nombre, i.codigo_sku, ai.cantidad_default as cantidad, i.stock_actual, i.unidad_medida, i.precio_costo as precio 
    ...
    public function createOT($data)
    {
        try {
            ...
            $sql = "INSERT INTO solicitudes_ot (
                        usuario_solicitante_id, activo_id, descripcion_trabajo, 
                        origen_tipo, area_negocio, centro_costo_ot, solicitante_externo, 
            ...
            if (!empty($data['items'])) {
                $sqlItem = "INSERT INTO detalle_solicitud (solicitud_id, insumo_id, cantidad, estado_linea) 
                            VALUES (:sid, :iid, :cant, 'PENDIENTE')";
                $stmtItem = $this->db->prepare($sqlItem);

                foreach ($data['items'] as $item) {
                    $stmtItem->execute([
                        ':sid' => $otId,
                        ':iid' => $item['insumo_id'] ?? $item['id'] ?? $item['id_producto'],
                        ':cant' => $item['cantidad']
                    ]);
                }
            }
    ...
    public function updateOT($id, $data)
    {
        try {
            ...
            if (!empty($idsParaBorrar)) {
                ...
                $this->db->prepare("DELETE FROM detalle_solicitud WHERE id IN ($placeholders)")
                    ->execute(array_values($idsParaBorrar));
            }
            ...
            foreach ($data['items'] as $item) {
                if (!empty($item['id_linea'])) {
                    ...
                    $stmtInsert->execute([
                        ':sid' => $id,
                        ':iid' => $item['insumo_id'] ?? $item['id_producto'],
                        ':cant' => $item['cantidad']
                    ]);
                }
            }
    ...
    public function getPendientesEntrega()
    {
        $sql = "SELECT ds.id as detalle_id, ds.cantidad, ds.cantidad_entregada,
                    ...
                    s.fecha_solicitud, i.id as insumo_id, i.nombre as insumo, i.codigo_sku, 
                    i.unidad_medida, i.stock_actual, s.id as ot_id, 
                    u.nombre as solicitante, u.apellido as solicitante_apellido, a.nombre as maquina
    ...
    public function getEntregasOT($otId)
    {
        $sql = "SELECT m.fecha, m.cantidad, i.nombre, i.codigo_sku, e.nombre_completo as receptor
                ...
                WHERE m.referencia_id IN (SELECT id FROM detalle_solicitud WHERE solicitud_id = :id) AND m.tipo_movimiento_id = 2
