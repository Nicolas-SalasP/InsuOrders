...
class InsumoRepository
{
    ...
    {
        $categorias = $this->db->query("SELECT id, nombre FROM categorias_insumo ORDER BY nombre")->fetchAll(PDO::FETCH_ASSOC);
        foreach ($categorias as &$cat) {
            $cat['nombre'] = mb_convert_case($cat['nombre'], MB_CASE_TITLE, "UTF-8");
        }

        return [
            'categorias' => $categorias,
            'sectores' => $this->db->query("SELECT * FROM sectores ORDER BY nombre")->fetchAll(PDO::FETCH_ASSOC),
            'ubicaciones' => $this->db->query("SELECT id, nombre, sector_id FROM ubicaciones ORDER BY nombre")->fetchAll(PDO::FETCH_ASSOC)
        ];
    }

    public function create($data)
    {
        $sql = "INSERT INTO insumos (codigo_sku, nombre, descripcion, categoria_id, ubicacion_id, stock_actual, stock_minimo, precio_costo, unidad_medida, moneda) 
                VALUES (:sku, :nom, :desc, :cat, :ubi, :stock, :min, :costo, :un, :mon)";
