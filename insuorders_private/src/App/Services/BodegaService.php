...
class BodegaService
{
    ...
    public function getPendientes() { return $this->mantencionRepo->getPendientesEntrega(); }
    public function getPorOrganizar() { return $this->repo->getPorOrganizar(); }
    public function getDevolucionesPendientes() { return $this->repo->getDevolucionesPendientes(); }
    ...
    public function organizar($insumoId, $ubicacionDestino, $cantidad)
    {
        ...
        if ($ubicacionOrigen == $ubicacionDestino) throw new Exception("La ubicación destino debe ser diferente a la de origen");
    ...
    {
        if (empty($motivo)) throw new Exception("Debe especificar un motivo para el rechazo.");
        $db = Database::getConnection();
        try {
            $db->beginTransaction();
