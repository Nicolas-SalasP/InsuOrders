...
const UsuarioModal = ({ show, onClose, onSave, usuarioEditar }) => {
    ...
    useEffect(() => {
        if (show) {
            // Cargar roles
            api.get('/index.php/usuarios/roles').then(res => {
                if(res.data.success) setRoles(res.data.data);
            });

            if (usuarioEditar) {
                setFormData({ ...usuarioEditar, password: '' }); // Password vacío al editar
            } else {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (usuarioEditar) {
                await api.put(`/index.php/usuarios?id=${usuarioEditar.id}`, formData);
            } else {
                await api.post('/index.php/usuarios', formData);
            }
            onSave();
            onClose();
        } catch (error) {
                ...
                <div className="modal-content shadow">
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="modal-body">
                            ...
                            <div className="row g-2 mb-3">
                                <div className="col-6">
                                    <label className="form-label small">Usuario (Login)</label>
                                    <input type="text" className="form-control" required disabled={!!usuarioEditar}
                                        value={formData.username} onChange={e => setFormData({...formData, username: e.target.value})} />
                                </div>
                                <div className="col-6">
                                    <label className="form-label small">Rol / Acceso</label>
                            ...
                            <div className="mb-3">
                                <label className="form-label small">
                                    {usuarioEditar ? 'Nueva Contraseña (Dejar en blanco para no cambiar)' : 'Contraseña *'}
                                </label>
